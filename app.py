...
with st.sidebar:
    ...
    if is_local:
        ...
        if _fresh_warns:
            st.warning(f"⏰ 再ビルド推奨: {' / '.join(_fresh_warns)}\n\n"
                       "`python scripts/build_corrected_time.py`\n"
                       "`python scripts/build_ltr_model.py`")
                    ...
                    try:
                        ...
                        if _ltr.available():
                            ...
                            for _, _lr in view_df.iterrows():
                                _lun = pd.to_numeric(_lr.get('Umaban'), errors='coerce')
                                if pd.isnull(_lun):
                                    continue
                                _lkt, _ = _jjh.resolve_horse(str(_lr.get('Name', '')))
                                _lsa = str(_lr.get('SexAge', ''))
                                _lsex = _lsa[:1] if _lsa and _lsa[0] in ('牡','牝','セ') else None
                                _lage = None
                                try:
                                    _lage = int(re.sub(r'\D', '', _lsa[1:])) if len(_lsa) > 1 else None
                                except Exception:
                                    pass
                                _lwh = str(_lr.get('WeightHistory', ''))
                                _lbw, _lzg = None, None
                                _lwm = re.match(r'(\d+)\(([+\-]?\d+)\)', _lwh)
                                if _lwm:
                                    _lbw, _lzg = int(_lwm.group(1)), int(_lwm.group(2))
                                _lft = None
                                try:
                                    _lft = float(str(_lr.get('WeightCarried', '')).replace('kg', ''))
                                except Exception:
                                    pass
                                _ltr_hs.append({
                                    'umaban': int(_lun), 'ketto_num': _lkt or '',
                                    'ninki': pd.to_numeric(_lr.get('Popularity'), errors='coerce'),
                            ...
                            if _ltr_sc:
                                ...
                                def _fmt_ltr_v(u):
                                    try:
                                        _uu = int(pd.to_numeric(u, errors='coerce'))
                                    except Exception:
                                        return '-'
                                    s = _ltr_sc.get(_uu)
                                    if s is None:
...
if nav == "🧠 MAGI回顧":
    ...
    <div class='magi-masthead'>
      <div class='magi-masthead-title'>🧠 MAGI SYSTEM — Retrospective Learning AI</div>
      <div class='magi-masthead-sub'>レース回顧・学習AI ／ MELCHIOR-1 × BALTHASAR-2 × CASPER-3</div>
    ...
    st.caption("レースが終わったら、その結果をMAGIの3人格（メルキオール／バルタザール／キャスパー）と"
               "おしゃべりしながら回顧します。会話から学んだことは回顧台帳に蓄積され、検証候補タグ・"
               "俗説隔離・履歴としてシステムの学習に活かされます。🏠 Single Race Analysis を先に解析する必要はありません。")
...
if nav == "🧠 MAGI回顧":
    ...
    try:
        ...
        _ledger_tags = mc.tag_summary()
        if _ledger_tags:
            _ready_t = {t: v for t, v in _ledger_tags.items() if v['ready']}
            _quarant = {t: v for t, v in _ledger_tags.items() if v['quarantined']}
            _growing = {t: v for t, v in _ledger_tags.items() if not v['ready'] and not v['quarantined']}
            _n_sess = len(mc._load_ledger())
            with st.expander(f"📊 回顧台帳ダイジェスト（{_n_sess}セッション・{len(_ledger_tags)}タグ蓄積）"):
                if _ready_t:
...
# --- レース回顧データベース（おしゃべり回顧の下に統合表示）---
if nav == "🧠 MAGI回顧":
    st.divider()
    st.header("📚 レース回顧データベース（登録・履歴・解析）")
    st.caption("上のおしゃべり回顧で「📒 審議を記録」すると、そのレースは自動でここの履歴に登録されます。"
               "過去レースをまとめて登録したり、解析結果と着順を並べて確認できます。")

    # 1. AI Guide (Updated for Learning Mode)
    with st.expander("✨ AIによる改善サイクルのやり方 (Learning Mode)", expanded=False):
        st.markdown("""
